...
public class FinderService {
	...
	// Managed repository ------------------------------
		...
		public Collection<Position> search(Finder finder){
			...
			minimumSalary=(finder.getMinimumSalary() == null ) ? 1000000
					: finder.getMinimumSalary();
			
			final Date maxDefaultDate = new GregorianCalendar(2200, Calendar.JANUARY, 1)
			.getTime();
			final Date minDefaultDate = new GregorianCalendar(1800, Calendar.JANUARY, 1)
			.getTime();
			
			maximumDeadline= finder.getMaximumDeadline() == null ? maxDefaultDate : finder
					.getMaximumDeadline();
		/*	if(finder.getDeadline().after(minDefaultDate) && finder.getDeadline().before(maxDefaultDate)){
				comprobar devolver fecha deadline y si es contenida en rango min y max devolver solo los resultados de ese rango
			}*/
			...
			if(finder.getDeadline()==null&&finder.getKeyWord().isEmpty()&&finder.getMinimumSalary()==null&&finder.getMaximumDeadline()==null){
				...
				results=this.finderRepository.search(keyWord, deadline, salary, maximumDeadline);
			}
			int count=0;
